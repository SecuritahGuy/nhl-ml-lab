import { buildFeaturesForGame } from "./features";
import { predict } from "./predict";
import {
  fetchGamecenterLanding, fetchScoreboardNow, fetchTeamRoster,
  fetchTeamsList, fetchGamecenterBoxscore,
} from "./nhl";
  ...
  async fetch(request: Request, env: any, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if (path === "/api/schedule") {
        ...
        const games = collectGames(data ?? schedData);
        return json({
          prev_date: data?.prevDate ?? schedData?.previousStartDate,
          current_date: data?.currentDate,
          next_date: data?.nextDate ?? schedData?.nextStartDate,
          games,
        });
      }
      ...
      if (schedTeamMatch) {
        const raw = await proxyFetch(`${WEB_API}/club-schedule-season/${schedTeamMatch[1].toUpperCase()}/now`);
        const games = (raw?.games ?? []).map(parseGame);
        return json({ team_abbrev: schedTeamMatch[1].toUpperCase(), season: raw?.currentSeason, games });
      }
    ...
    async function handleLineups(gameId: number): Promise<Response> {
      ...
      const game = {
        ...
        home_team: {
          id: landing.homeTeam?.id,
          abbrev: landing.homeTeam?.abbrev,
          place_name: landing.homeTeam?.placeName?.default ?? "",
        },
        away_team: {
          id: landing.awayTeam?.id,
          abbrev: landing.awayTeam?.abbrev,
          place_name: landing.awayTeam?.placeName?.default ?? "",
        },
      };
